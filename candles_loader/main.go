...
import (
	...
	sdk "../thirdparty/invest-openapi-go-sdk"
...
func main() {
	...
	err := gonfig.GetConf(*configurationFilePathPtr, &configuration)
	// Check error
	if err != nil {
		log.Fatalln(err)
	}
	...
	// 	rest()
	// }
	globalRanks, err := readGlobalRankParquetr("../data/companies_rank.csv")
	if err != nil {
		log.Fatalln(err)
	}
...
func sandboxRest() {
	...
	// Get Candles for 01/01/2020
	// Init start of period
	timeFrom, err := time.Parse(
		time.RFC3339,
	...
	// Try to get candles
	candles, err := client.RestClient.Candles(context.Background(), timeFrom, timeTo, "15min", "BBG005DXJS36")
	// Check error
	if err != nil {
		log.Fatalln(err)
	}
	...
	// Print candles
	log.Printf("Received candles: %+v", candles)
